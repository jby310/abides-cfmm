...
class Kernel:
  ...
  def runner(self, agents = [], startTime = None, stopTime = None,
             ...
             seed = None, oracle = None, log_dir = None, output_mid_price_path=None):
    ...
    for sim in range(num_simulations):
      ...
      '''
      这里的message是什么时候被填充的呢?——在kernelStarting的时候，最终是在kernel的setWakeup函数中填充的。
      '''
    ...
    # log itself.
    new_mid_price = self.writeSummaryLog(output_mid_price_path)
    ...
    return self.custom_state, new_mid_price
  ...
  def writeSummaryLog (self, output_mid_price_path):
    path = os.path.join(".", "log", self.log_dir)
    ...
    def extract_mid_price_to_csv(exchange_path, ob_path, output_csv_path, new_col_name="mid_price"):
        """
        从ABIDES的交易所输出和订单簿文件中提取mid_price，按时间戳保存为CSV（支持增量添加列）。
        
        参数：
            exchange_path: ABIDES交易所输出文件路径（如ExchangeAgent.bz2）
            ob_path: ABIDES订单簿输出文件路径（如ORDERBOOK_TICKER_FULL.bz2）
            output_csv_path: 输出CSV的保存路径
            new_col_name: 新添加的mid_price列名（默认"mid_price"，重复时自动加后缀）
        """
        # 构建并清洗订单簿数据（复用原代码逻辑）
        print("正在构建订单簿数据...")
        ...
        # 过滤无效的中间价（排除流动性崩溃的异常值）
        cleaned_orderbook = processed_orderbook[
            (processed_orderbook['MID_PRICE'] > -MID_PRICE_CUTOFF) &
            (processed_orderbook['MID_PRICE'] < MID_PRICE_CUTOFF)
        ]
        
        # 提取时间戳和mid_price，整理为DataFrame
        new_mid_price = cleaned_orderbook[['MID_PRICE']].reset_index()
        new_mid_price.columns = ['timestamp', f'{new_col_name}_1']  # 用自定义列名
        
        new_mid_price.to_csv(output_csv_path, index=False, encoding='utf-8')
        print(f"新文件已创建：{output_csv_path}")
        print(f"共保存 {len(new_mid_price)} 条有效记录（列名：{new_col_name}）")
